...
int init_simple_filtergraph(InputStream *ist, OutputStream *ost)
{
	...
	av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in init_simple_filtergraph, ist->nb_filters = %d.\n", 
		ist->nb_filters);
    ist->filters[ist->nb_filters - 1] = fg->inputs[0];

    GROW_ARRAY(filtergraphs, nb_filtergraphs);
	av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in init_simple_filtergraph, nb_filtergraphs = %d.\n", 
		nb_filtergraphs);
...
int configure_output_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_output_filter, avfilter_pad_get_type = ");
    switch (avfilter_pad_get_type(out->filter_ctx->output_pads, out->pad_idx)) {
    case AVMEDIA_TYPE_VIDEO: av_log(NULL, AV_LOG_INFO, "AVMEDIA_TYPE_VIDEO.\n");return configure_output_video_filter(fg, ofilter, out);
    case AVMEDIA_TYPE_AUDIO: av_log(NULL, AV_LOG_INFO, "AVMEDIA_TYPE_AUDIO.\n");return configure_output_audio_filter(fg, ofilter, out);
...
#define AUTO_INSERT_FILTER_INPUT(opt_name, filter_name, arg) do {                 \
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_input_audio_filter, audio_sync_method = %d.\n", audio_sync_method);
...
//         av_bprint_finalize(&pan_buf, NULL);
//     }
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_input_audio_filter, audio_volume = %d.\n", audio_volume);
    if (audio_volume != 256) {
        char args[256];
                                  ...
                                  AVFilterInOut *in)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_input_filter, avfilter_pad_get_type = ");
    switch (avfilter_pad_get_type(in->filter_ctx->input_pads, in->pad_idx)) {
    case AVMEDIA_TYPE_VIDEO: av_log(NULL, AV_LOG_ERROR, "AVMEDIA_TYPE_VIDEO.\n");return configure_input_video_filter(fg, ifilter, in);
    case AVMEDIA_TYPE_AUDIO: av_log(NULL, AV_LOG_ERROR, "AVMEDIA_TYPE_AUDIO.\n");return configure_input_audio_filter(fg, ifilter, in);
    default: av_assert0(0);
    }
}
...
int configure_filtergraph(FilterGraph *fg)
{
    ...
    if (!(fg->graph = avfilter_graph_alloc()))
        return AVERROR(ENOMEM);
    av_log(NULL, AV_LOG_INFO, "\npanpan test, in configure_filtergraph start, simple = %d, graph_desc = %s.\n", simple, graph_desc);
    if (simple) {
        ...
        av_log(NULL, AV_LOG_INFO, "panpan test, in configure_filtergraph 3, args = %s.\n", args);
        args[0] = 0;
        ...
        av_log(NULL, AV_LOG_INFO, "panpan test, in configure_filtergraph 4, args = %s.\n", args);
        ...
        av_log(NULL, AV_LOG_INFO, "panpan test, in configure_filtergraph 5, args = %s.\n", args);

        e = av_dict_get(ost->encoder_opts, "threads", NULL, 0);
        if (e)
        {
            av_log(NULL, AV_LOG_INFO, "panpan test, in configure_filtergraph 6, e->value = %s.\n", e->value);
    ...
    if(!inputs)
    {
         av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_filtergraph, inputs = null.\n");
     }
    if(!outputs)
    {
         av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_filtergraph, outputs = null.\n");
     }
    if (hw_device_ctx) {
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_filtergraph, go to configure_input_filter.\n");
    ...
    avfilter_inout_free(&inputs);
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_filtergraph, inputs, i = %d.\n", i);
    for (cur = outputs, i = 0; cur; cur = cur->next, i++)
        configure_output_filter(fg, fg->outputs[i], cur);
    av_log(NULL, AV_LOG_ERROR, "panpan test, in configure_filtergraph, outputs, i = %d.\n", i);
    avfilter_inout_free(&outputs);
    ...
    av_log(NULL, AV_LOG_INFO, "panpan test, in configure_filtergraph end.\n\n");
    return 0;
}

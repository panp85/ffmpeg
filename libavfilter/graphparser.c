                         ...
                         const char *filt_name, const char *args, void *log_ctx)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in create_filter, inst_name = %s.\n", inst_name);
    filt = avfilter_get_by_name(filt_name);

    if (!filt) {
        av_log(NULL, AV_LOG_ERROR, "panpan test, in create_filter, filt = null.\n");
        av_log(log_ctx, AV_LOG_ERROR,
               "No such filter: '%s'\n", filt_name);
                        ...
                        int index, void *log_ctx)
{
    ...
    int ret;
    av_log(NULL, AV_LOG_ERROR, "panpan test, in parse_filter, name = %s.\n", name);
    if (**buf == '=') {
        (*buf)++;
        opts = av_get_token(buf, "[],;");
    }
    av_log(NULL, AV_LOG_ERROR, "panpan test, in parse_filter, opts = %s.\n", opts);
...
{
    int pad, ret;
    av_log(NULL, AV_LOG_ERROR, "panpan test, in link_filter_inouts, filt_ctx->nb_inputs = %d.\n", filt_ctx->nb_inputs);
    for (pad = 0; pad < filt_ctx->nb_inputs; pad++) {
        AVFilterInOut *p = *curr_inputs;
        av_log(NULL, AV_LOG_ERROR, "panpan test, in link_filter_inouts, in for, pad = %d.\n", pad);
        if (p) {
            *curr_inputs = (*curr_inputs)->next;
            p->next = NULL;
        } else if (!(p = av_mallocz(sizeof(*p))))
            return AVERROR(ENOMEM);
        av_log(NULL, AV_LOG_ERROR, "panpan test, in link_filter_inouts, p->filter_ctx = %s.\n", p->filter_ctx ? "yes" : "null");
        if (p->filter_ctx) {
            ...
            av_log(NULL, AV_LOG_ERROR, "panpan test, in link_filter_inouts, append_inout to open_inputs.\n", p->filter_ctx ? "yes" : "null");
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in link_filter_inouts, filt_ctx->nb_outputs = %d.\n", filt_ctx->nb_outputs);
                         ...
                         AVFilterInOut **open_outputs, void *log_ctx)
{
    ...
    while (**buf == '[') {
        ...
        *curr_inputs = (*curr_inputs)->next;
        ...
        if (match) {
            av_log(NULL, AV_LOG_ERROR, "panpan test, in parse_outputs, match yes name = %s.\n", name);
            ...
            av_log(NULL, AV_LOG_ERROR, "panpan test, in parse_outputs, input->name = %s, go to insert_inout.\n", name);
                          ...
                          AVFilterInOut **outputs)
{
    ...
    do {
        ...
        filters += strspn(filters, WHITESPACES);
        av_log(NULL, AV_LOG_ERROR, "panpan test, in avfilter_graph_parse2, filters = %s.\n", filters);
        if ((ret = parse_inputs(&filters, &curr_inputs, &open_outputs, graph)) < 0)
            goto end;
        av_log(NULL, AV_LOG_ERROR, "panpan test, in avfilter_graph_parse2, curr_inputs = %s, open_outputs = %s.\n", curr_inputs, open_outputs);
        ...
        filters += strspn(filters, WHITESPACES);
        chr = *filters++;
       // av_log(NULL, AV_LOG_ERROR, "panpan test, in avfilter_graph_parse2, chr = %c, curr_inputs %s.\n", chr, curr_inputs ? "yes" : "no");
        if (chr == ';' && curr_inputs)
        {
            av_log(NULL, AV_LOG_ERROR, "panpan test, in avfilter_graph_parse2, curr_inputs->name = %s, go to append_inout.\n", curr_inputs->name);

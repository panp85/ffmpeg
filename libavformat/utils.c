                      ...
                      AVDictionary **options)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in init_input, s->pb : %s.\n", s->pb ? "yes" : "no");
    if (s->pb) {
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in init_input 2, s->iformat : %s.\n", s->iformat ? "yes" : "no");
    if ((s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format2(&pd, 0, &score))))
        return score;
    av_log(NULL, AV_LOG_ERROR, "panpan test, in init_input 3, go to io_open.\n");
    ...
    for (;;) {
        ...
        for (i = 0; i < ic->nb_streams; i++) {
            ...
            if (!(st->r_frame_rate.num && st->avg_frame_rate.num) &&
                ...
                {
                    av_log(NULL, AV_LOG_ERROR, 
						"panpan test, in avformat_find_stream_info, break, count < fps_analyze_framecount.\n");
                    break;
                }
            }
            ...
            {
                av_log(NULL, AV_LOG_ERROR, 
						"panpan test, in avformat_find_stream_info, break, st->parser && st->parser->parser->split && !st->internal->avctx->extradata.\n");
                break;
            }
			if (st->first_dts == AV_NOPTS_VALUE &&
			...
			{
			    av_log(NULL, AV_LOG_ERROR, 
						"panpan test, in avformat_find_stream_info, break, 3.\n");
                break;
			}
        }
        ...
        if (read_size >= probesize) {
			...
			av_log(NULL, AV_LOG_ERROR, 
				"panpan test, in avformat_find_stream_info, break, read_size >= probesize.\n");
            break;
        }
        ...
        if (st->codec_info_nb_frames>1) {
            ...
            if (t >= limit) {
                ...
                if (ic->flags & AVFMT_FLAG_NOBUFFER)
                    av_packet_unref(pkt);
				av_log(NULL, AV_LOG_ERROR, 
				    "panpan test, in avformat_find_stream_info, break, t >= limit.\n");
                break;
            }
            if (pkt->duration) {
    ...
    if (flush_codecs) {
        ...
        for (i = 0; i < ic->nb_streams; i++) {
            ...
            if (st->info->found_decoder == 1) {
                ...
                do {
					av_log(NULL, AV_LOG_ERROR, 
						"panpan test, in avformat_find_stream_info, go to try_decode_frame x = %d.",
						x);

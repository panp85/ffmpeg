...
static int init_input_stream(int ist_index, char *error, int error_len)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in init_input_stream, ist->decoding_needed : %s.\n", ist->decoding_needed ? "yes" : "no");
    if (ist->decoding_needed) {
...
static int transcode_init(void)
{
    ...
    av_log(NULL, AV_LOG_INFO, "panpan test, in transcode_init, nb_output_streams = %d.\n", nb_output_streams);
...
//		av_log(NULL, AV_LOG_ERROR, "panpan test, in transcode_init, ost = %d.\n", ost);
        ...
        av_log(NULL, AV_LOG_INFO, "panpan test, in transcode_init, ost->stream_copy = %d.\n", ost->stream_copy);
        if (ost->stream_copy) {
            AVRational sar;
            uint64_t extra_size;
            ...
            if ((enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
                 ...
                 filtergraph_is_simple(ost->filter->graph)) {
                    av_log(NULL, AV_LOG_INFO, "\npanpan test, in transcode_init, go to configure_filtergraph.\n");
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
        ...
        av_log(NULL, AV_LOG_ERROR, "panpan test, in transcode_init, ost->disposition = %s.\n", ost->disposition);
        if (ost->disposition) {
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in transcode_init, nb_input_streams = %d.\n", nb_input_streams);
    /* init input streams */
    for (i = 0; i < nb_input_streams; i++)
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in transcode_init, nb_output_streams = %d.\n", nb_output_streams);
    /* open each encoder */
    for (i = 0; i < nb_output_streams; i++) {
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in transcode_init, nb_input_files = %d.\n", nb_input_files);
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in transcode_init, nb_output_files = %d.\n", nb_output_files);

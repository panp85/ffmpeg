...
{\
    ...
    for (i = 0; i < o->nb_ ## name; i++) {\
        ...
        av_log(NULL, AV_LOG_ERROR, "panpan test, in MATCH_PER_STREAM_OPT, spec = %s.\n", spec);\
...
{
    int i, ret;
    av_log(NULL, AV_LOG_INFO, "panpan test, in add_input_streams, ic->nb_streams = %d.\n", ic->nb_streams);
    for (i = 0; i < ic->nb_streams; i++) {
...
static void assert_file_overwrite(const char *filename)
{
    ...
    av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in assert_file_overwrite, filename = %s, file_overwrite = %d.\n",
		filename, file_overwrite);
    if (!file_overwrite) {
...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in open_input_file, o->nb_audio_sample_rate %d.\n", o->nb_audio_sample_rate);
    if (o->nb_audio_sample_rate) {
        av_dict_set_int(&o->g->format_opts, "sample_rate", o->audio_sample_rate[o->nb_audio_sample_rate - 1].u.i, 0);
    }
    av_log(NULL, AV_LOG_ERROR, "panpan test, in open_input_file, o->nb_audio_channels = %d.\n", o->nb_audio_channels);
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in open_input_file, o->nb_frame_rates = %d.\n", o->nb_frame_rates);
    if (o->nb_frame_rates) {
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in open_input_file, o->nb_frame_sizes = %d.\n", o->nb_frame_sizes);
    if (o->nb_frame_sizes) {
        av_dict_set(&o->g->format_opts, "video_size", o->frame_sizes[o->nb_frame_sizes - 1].u.str, 0);
    }
    av_log(NULL, AV_LOG_ERROR, "panpan test, in open_input_file, o->nb_frame_pix_fmts = %d.\n", o->nb_frame_pix_fmts);
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in open_input_file, video_codec_name, audio_codec_name, subtitle_codec_name, data_codec_name = [%s, %s, %s, %s].\n", video_codec_name, audio_codec_name, subtitle_codec_name, data_codec_name);
    ...
    av_log(NULL, AV_LOG_INFO, "panpan test, in open_input_file, go to add_input_streams.\n");
...
static int choose_encoder(OptionsContext *o, AVFormatContext *s, OutputStream *ost)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO || type == AVMEDIA_TYPE_SUBTITLE) {
		...
		av_log(NULL, AV_LOG_FATAL, "panpan test, in choose_encoder, codec_name = %s.\n", codec_name);
        if (!codec_name) {
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    double qscale = -1;
    int i;
    av_log(NULL, AV_LOG_ERROR, "panpan test, in new_output_stream, go in.\n");
    if (!st) {
    ...
    if (ost->enc) {
		...
		av_log(NULL, AV_LOG_ERROR, "panpan test, in new_output_stream, preset = %s.\n", preset);
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
	...
	av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in new_video_stream, frame_rate = %s.\n",
		frame_rate);
	...
	av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in new_video_stream, frame_aspect_ratio = %s.\n",
		frame_aspect_ratio);
	...
	av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in new_video_stream, ost->filters_script = %s.\n",
		ost->filters_script);
    MATCH_PER_STREAM_OPT(filters,        str, ost->filters,        oc, st);
	av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in new_video_stream, ost->filters = %s.\n",
			ost->filters);
    ...
    if (!ost->stream_copy) {
		...
		av_log(NULL, AV_LOG_ERROR, "panpan test, in new_video_stream, frame_size = %s.\n", frame_size);
		...
		av_log(NULL, AV_LOG_ERROR, "panpan test, in frame_pix_fmt, frame_size = %s.\n", frame_pix_fmt);
		...
		av_log(NULL, AV_LOG_ERROR, "panpan test, in frame_pix_fmt, intra_matrix = %s.\n", intra_matrix);
        if (intra_matrix) {
		...
		av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in frame_pix_fmt, chroma_intra_matrix = %s.\n", 
			chroma_intra_matrix);
		...
		av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in frame_pix_fmt, inter_matrix = %s.\n", 
			inter_matrix);
        if (inter_matrix) {
...
static OutputStream *new_audio_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "panpan test, in new_audio_stream, go in.\n");
...
static int open_output_file(OptionsContext *o, const char *filename)
{
    ...
    av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in open_output_file, nb_filtergraphs = %d.\n", 
		nb_filtergraphs);
    ...
    if (!strcmp(file_oformat->name, "ffm") && !override_ffserver &&
        ...
        if (!o->video_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_VIDEO) != AV_CODEC_ID_NONE) {
            ...
            {
                av_log(NULL, AV_LOG_ERROR, 
					"panpan test, in open_output_file, video, go to new_video_stream.\n");
        ...
        if (!o->audio_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_AUDIO) != AV_CODEC_ID_NONE) {
            ...
            {
                av_log(NULL, AV_LOG_ERROR, 
					"panpan test, in open_output_file, audio, go to new_audio_stream.\n");
		...
		av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in open_output_file, subtitle_codec_name = %s.\n", 
			subtitle_codec_name);
        ...
        if (!o->data_disable ) {
			...
			av_log(NULL, AV_LOG_ERROR, 
			    "panpan test, in open_output_file, data enable codec_id = %d.\n", 
			    codec_id);
...
loop_end:
    ...
    av_log(NULL, AV_LOG_ERROR, 
			    "panpan test, in open_output_file, o->nb_attachments = %d.\n", o->nb_attachments);
    ...
    av_dict_free(&unused_opts);
    av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in open_output_file, of->ost_index = %d, nb_output_streams = %d.\n", 
		of->ost_index, nb_output_streams);
    /* set the decoding_needed flags and create simple filtergraphs */
    for (i = of->ost_index; i < nb_output_streams; i++) {
        OutputStream *ost = output_streams[i];
        av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in open_output_file, ost->source_index = %d.\n",
			ost->source_index);
    ...
    {
        av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in open_output_file, AVFMT_NOFILE yes, image2.\n");
        assert_file_overwrite(filename);
    }else
    {
        av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in open_output_file, AVFMT_NOFILE yes 3.\n");
    }
    if (o->mux_preload) {
		av_log(NULL, AV_LOG_ERROR, 
			"panpan test, in open_output_file, o->mux_preload = %f.\n", o->mux_preload);
        av_dict_set_int(&of->opts, "preload", o->mux_preload*AV_TIME_BASE, 0);
    }
    oc->max_delay = (int)(o->mux_max_delay * AV_TIME_BASE);
    av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in open_output_file, o->nb_metadata_map = %d, o->chapters_input_file = %d, nb_input_files = %d.\n", 
		o->nb_metadata_map, o->chapters_input_file, nb_input_files);
    /* copy metadata */
    ...
    av_log(NULL, AV_LOG_ERROR, 
		"panpan test, in open_output_file, o->nb_program = %d, o->nb_metadata = %d.\n",
		o->nb_program, o->nb_metadata);
